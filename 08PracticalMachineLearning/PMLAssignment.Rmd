...
|Random Tree *rf*|**99.6%**|    75/25     | 52 | ntree=10 | 131  | 
...
t1<-Sys.time()
modelFitRF3<-train(classe~roll_belt+pitch_belt+yaw_belt+total_accel_belt,method="rf",ntree=100,data=RFtrain)
t2<-Sys.time()
...
RFtest<-pml_training2[-inTrain,]
t1<-Sys.time()
modelFitBoost<-gbm(classe~.,n.trees=300,data=RFtrain,distribution="multinomial",shrinkage = .7,verbose=FALSE)
t2<-Sys.time()
